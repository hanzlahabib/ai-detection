	...
	class JNews_Social_Login {
		...
		public function linkedin_oauth_url() {
			...
			$authUrl = $linkedin->getLoginUrl(
				array(
					LinkedIn\LinkedIn::SCOPE_OPEN_ID,
		...
		public function linkedin_login() {
			...
			try {
				...
				$info       = $linkedin->get_profile( $token );
		...
		public function register_user_social( $user ) {
			if ( ! apply_filters( 'jnews_social_login_force_disable', false ) ) {
				...
				$username            = $this->jnews_generate_username( $user );
		...
		/**
		 * Generate a random username based on firstname and last name users
		 *
		 * @param array|string $user User data.
		 * @return string
		 */
		public function jnews_generate_username( $user ) {
			/* Check if $user come from register_user_social function or from recursive */
			if ( is_array( $user ) ) {
				if ( isset( $user['first_name'] ) ) {
					$first_name = str_replace( ' ', '', $user['first_name'] );
					$first_name = strtolower( preg_replace( '/[^A-Za-z0-9\-]/', '', $first_name ) );
				...
				if ( isset( $user['last_name'] ) ) {
					...
					$last_name = strtolower( preg_replace( '/[^A-Za-z0-9\-]/', '', $last_name ) );
				} else {
					$last_name = '';
				}
				/* Use email in case if first & last name empty */
			...
			if ( username_exists( $username ) ) {
				$this->jnews_generate_username( substr( $username, 0, -5 ) );
			}
			return $username;
		}
