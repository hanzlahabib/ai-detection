...
class LinkedIn {
	...
	const SCOPE_OPEN_ID            = 'openid'; // get Open ID Token Acces
	const SCOPE_BASIC_PROFILE      = 'profile'; // Name, photo, headline, and current positions
	const SCOPE_FULL_PROFILE       = 'r_fullprofile'; // Full profile including experience, education, skills, and recommendations
	const SCOPE_EMAIL_ADDRESS      = 'email'; // The primary email address you use for your LinkedIn account
	...
	public function get_profile( $token ) {
		...
		$response = wp_remote_get( 'https://api.linkedin.com/v2/userinfo', $args );
		if ( ! is_wp_error( $response ) ) {
			$result = wp_remote_retrieve_body( $response );
			$result = json_decode( $result, true );
			return $result;
		} else {
			throw new Exception( '<strong>' . esc_html__( 'Error', 'jnews-social-login' ) . ':</strong> ' . esc_html__( 'Failed to get Linked in User Profile', 'jnews-social-login' ) );
		}
	}
