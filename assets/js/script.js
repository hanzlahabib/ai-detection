(function($) {
	...
	const circle = document.querySelector('.mouse-circle-mover');
	const follow = document.querySelector('.mouse-circle-mover-follow');
	...
	function moveCircle(e) {
		...
		circle.classList.add('active'); // add active class to trigger animation

		// Update the position of the follow circle
		follow.style.transform = `translate(${x}px, ${y}px)`;
		follow.classList.add('active'); // add active class to trigger animation

		// Reset the active classes after the transition
		setTimeout(() => {
   ...
   /* $(document).ready(function() {
		...
		function moveCircle(e) {
			...
			gsap.to($circle, { duration: 1, x: e.clientX, y: e.clientY });
			// Move the larger follow circle
			gsap.to($follow, { duration: 1.5, x: e.clientX, y: e.clientY });
		}
